...
class OnMemberJoinCog(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        if member.guild.id == 766213910595633153:
            ...
            idraw.text((755.5, 680), f'{self.bot.get_guild(766213910595633153).name}', anchor="ms", font=footer, fill='#FFFFFF')
            # Маска для картинки
            ...
            await message_channel.send(f"__**Приветствуем нового участника**__: {member.mention}!", file=discord.File(fp=_buffer, filename=f'{member.name}welcome.png'))
