...
from config import bot_initialize, bot_settings
...
class ModerationCog(commands.Cog):
    ...
    @commands.command(aliases=['кик', 'кикни', 'выгони', 'кикнуть', 'выгнать'])
    ...
    async def kick(self, ctx, member: discord.Member = None, reason=None):
        ...
        if member is None:
            await ctx.send('Укажите в аргумент @пользователя, которого необходимо выгнать с сервера!', delete_after=10)
        elif member is ctx.message.author:
            await ctx.send("Я не позволю Вам выгнать самого себя!\n", delete_after=10)
        else:
            if reason is None:
    ...
    @commands.command(aliases=['удалить', 'удали', 'очистить', 'очисти'])
    @commands.has_any_role(766233124681547776, 766231587104620554)  # Support, # Owner
    ...
    async def clear(self, ctx, amount: str = None):
        if not amount:
            ...
            if int(amount) > 100:
                ...
                if amount is None:
                    await ctx.send('Укажите в аргумент количество сообщений, которые необходимо удалить!',
                                   delete_after=10)
                else:
    ...
    @commands.command(aliases=['добавить_роль', 'добавь_роль', 'выдай_роль', 'выдать_роль'])
    @commands.has_any_role(766233124681547776, 766231587104620554)  # Support, # Owner
    async def add_role(self, ctx, member: discord.Member = None, role: discord.Role = None):
        logs = self.bot.get_channel(bot_settings['log_channel'])
        await ctx.message.delete()
        if member == None:
            await ctx.send('Укажите в аргумент @пользователя и @роль, которую необходимо выдать пользователю!',
                           delete_after=10)
        elif role == None:
            await ctx.send('Укажите в аргумент @пользователя и @роль, которую необходимо выдать пользователю!',
                           delete_after=10)
    ...
    @commands.command(aliases=['забрать_роль', 'забери_роль'])
    @commands.has_any_role(766233124681547776, 766231587104620554)  # Support, # Owner
    async def remove_role(self, ctx, member: discord.Member = None, role: discord.Role = None):
        logs = self.bot.get_channel(bot_settings['log_channel'])
        await ctx.message.delete()
        if member == None:
            await ctx.send('Укажите в аргумент @пользователя и @роль, у которую необходимо забрать роль!',
                           delete_after=10)
        elif role == None:
            await ctx.send('Укажите в аргумент @пользователя и @роль, у которого необходимо забрать роль!',
                           delete_after=10)
        else:
    ...
    @commands.command(aliases=['бан', 'забань', 'заблокируй', 'забанить', 'заблокировать'])
    @commands.has_any_role(766233124681547776, 766231587104620554)  # Support, # Owner
    async def ban(self, ctx, member: discord.Member = None, reason=None):
        ...
        if member is None:
            ...
            await ctx.send("Я не позволю Вам заблокировать самого себя!\n", delete_after=10)
        else:
            if reason is None:
    ...
    @commands.command(aliases=['мут', 'замуть', 'затки', 'замутить', 'заткнуть'])
    @commands.has_any_role(766233124681547776, 766231587104620554)  # Support, # Owner
    async def mute(self, ctx, member: discord.Member = None):
        logs = self.bot.get_channel(bot_settings['log_channel'])
        await ctx.message.delete()
        if member == None:
            await ctx.send('Укажите в аргумент @пользователя, которому необходимо ограничить доступ общения!',
                           delete_after=10)
        elif member == ctx.message.author:
            await ctx.send("Я не позволю Вам заглушить самого себя!\n", delete_after=10)
        else:
    ...
    @commands.command(aliases=['анмут', 'размуть', 'размут', 'сними_мут'])
    @commands.has_any_role(766233124681547776, 766231587104620554)  # Support, # Owner
    async def unmute(self, ctx, member: discord.Member = None):
        logs = self.bot.get_channel(bot_settings['log_channel'])
        await ctx.message.delete()
        if member == None:
            ctx.send("Укажите в аргумент @пользователя, которому необходимо вернуть доступ к каналам связи!",
                     delete_after=10)
        else:
